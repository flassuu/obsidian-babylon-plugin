...
export class GenerateTemplateModal extends Modal {
	...
	// when set, generates a body-only template with the preset's property aliases
	...
	private buildTemplateContent(settings: { selectedFields?: string[]; customFieldNames?: string[] }): string {
		if (this.preset) {
			return this.buildPresetBodyContent();
		}
	...
	// body-only template: the preset owns the frontmatter, so this file only
	// contains placeholders that use the preset's property names
	private buildPresetBodyContent(): string {
		const preset = this.preset!;
		const props = [...preset.fields]
		...
		const lines: string[] = [
			...
			this.language === 'en'
				? '# Placeholders use the preset property names'
				: '# Плейсхолдеры используют имена полей из пресета',
			'',
		];

		lines.push(has('title') ? '# {{title}}' : '# Note title');
		lines.push('');
		if (has('description')) {
			lines.push('> {{description}}', '');
		}
		if (has('year') || has('format') || has('episodes')) {
			const parts: string[] = [];
			if (has('year')) parts.push(`**${this.language === 'en' ? 'Year' : 'Год'}:** {{year}}`);
			if (has('format')) parts.push(`**${this.language === 'en' ? 'Format' : 'Формат'}:** {{format}}`);
			if (has('episodes')) parts.push(`**${this.language === 'en' ? 'Episodes' : 'Эпизодов'}:** {{episodes}}`);
			lines.push(parts.join(' | '), '');
		}
		if (has('siteUrl')) {
			lines.push('[View on AniList]({{siteUrl}})', '');
		}
